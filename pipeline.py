...
if StrictVersion(seesaw.__version__) < StrictVersion('0.10.4'):
    raise Exception('This pipeline needs seesaw version 0.10.4 or higher.')
...
LUA_SHA1 = get_hash(os.path.join(CWD, 'freeml.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--warc-header', 'freeml-dld-script-version: ' + VERSION,
            '--warc-header', ItemInterpolation('freeml-item: %(item_name)s')
        ]
        ...
        if item_type == 'users':
            ...
            for i in range(int(start), int(end)+1):
                wget_args.extend(['--warc-header', 'freeml-user-id:' + str(i)])
                wget_args.append('https://www.freeml.com/ep.umzx/grid/MLC/node/MlcHomeFront/mlc_id/' + str(i))
                wget_args.append('https://www.freeml.com/bl/{}/'.format(i))
        else:
